  ...
  var defConfig = {
      ...
      checkLotters : ['<div class="bd game-submit-confirm-cont">',
                  '<p class="game-submit-confirm-title">',
                    ...
                    '<label class="ui-label" style="display:none;" id="issue-for-print">奖期：<#=lotteryIssue#></label>',
                  '</p>',
                  '<ul class="ui-form">',
                    '<li>',
      ...
      successTip : ['<div class="bd text-center">',
                '<div class="pop-title">',
                  ...
                  '<h4 class="pop-text"><#=msg#><br /></h4>',
                '</div>',
                '<p class="text-note" style="padding:5px 0;">您可以通过”<a href="<#=link#>">游戏记录</a>“查询您的投注记录！</p>',
              '</div>'].join(''),
      //部分投注成功
      betPart : ['<div class="bd text-center">',
            ...
            '</div>'].join(''),
      //投注失败
      betFailed : ['<div class="bd text-center">',
                '<div class="pop-waring">',
                  '<i class="ico-waring <#=icon-class#>"></i>',
                  '<h4 class="pop-text"><#=msg#><br /></h4>',
                  '<div class="bet-failed-content"><#=failed_html#></div>',
                '</div>',
              '</div>'].join(''),
      //提醒选求提示
      ...
      blockade : ['<div class="bd panel-game-msg-blockade" id="J-blockade-panel-main">',
              '<form id="J-form-blockade-detail" action="ssc-blockade-detail.php" method="post"></form>',
  ...
  var pros = {
    ...
    rebuilderrorTip :function(parameter){
      var me = this, result = {};
        ...
        result['dialogType'] = BootstrapDialog.TYPE_WARNING;
    ...
    rebuildnormal: function(parameter){
      var me = this, result = {};
        ...
        result['dialogType'] = BootstrapDialog.TYPE_INFO;
        result['content'] = me.formatHtml(parameter['tpl'], parameter['data']['tplData']);
        me.show($.extend(result, parameter));
    },
    //获取默认提示弹窗
    ...
    rebuildbet_failed:function(parameter){
      var me = this, result = {}, failed_html = [];
        result['mask'] = true;
        result['iconClass'] = '';
        result['closeIsShow'] = true;
        result['closeFun'] = function(){
        ...
        result['dialogType'] = BootstrapDialog.TYPE_WARNING;
        $.each(parameter['data']['tplData']['failed'], function(idx, failed){
          failed_html.push('<div class="bet-failed-list">');
            failed['way_name'] && failed_html.push('<p class="bet-failed-name">[' +failed['way_name'] + ']</p>');
            failed_html.push('<p class="bet-failed-detail">' +failed['ball']+ '</p>');
            failed_html.push('<p class="bet-failed-reason c-red">' +failed['reason']+ '</p>');
          failed_html.push('</div>')
        });
        parameter['data']['tplData']['failed_html'] = failed_html.join('');
        result['content'] = me.formatHtml(parameter['tpl'], parameter['data']['tplData']);
        me.show($.extend(result, parameter));
    },
    getHtmlbet_failed:function(){
      var cfg = this.defConfig;
      return cfg.betFailed;
    },
    /*
      //彩种核对
      betgame.Games.getCurrentGameMessage().show({
         type : 'checkLotters',
         data : {
            tplData : {
              //当期彩票详情
    ...
    rebuildsuccess : function(parameter){
      var me = this, result = {};
        ...
        result['dialogType'] = BootstrapDialog.TYPE_SUCCESS;
        result['content'] = me.formatHtml(parameter['tpl'], parameter['data']['tplData']);
        me.show($.extend(result, parameter));
    },
    ...
    show: function(data){
      var me = this, dialog = me.getDialog();
      me.reSet();
      ...
      dialog.setTitle(data['title'] || '温馨提示')
            ...
            .setType(data['dialogType'] || BootstrapDialog.TYPE_INFO)
            .open();
...
})(betgame, "GameMessage",  betgame.Event);
