...
(function(host, name, Event, undefined) {
  var defConfig = {
      //事件监听容器
      containerEvent: '[data-order-detail]',
      //主面板dom
      containerDom: '[data-order-detail]',
      //总注数dom
      totalLotterysNumDom: '[data-order-num]',
      //选择的奖金组
      totalBounsLists: '#J-bouns-lists-value',
      //总金额dom
      totalAmountDom: '[data-order-money]',
      //总返点dom
      ...
      //       +' </li>',
      rowTemplate: '<li id="gameorder-<#=id#>">'//'<li data-param="action=reselect&id=<#=id#>&mid=<#=mid#>" id="gameorder-<#=id#>">'
            +'  <span data-param="action=reselect&id=<#=id#>" class="number" title="<#=lotterysText#>"><#=lotterysText#></span>'
            +'  <span data-param="action=reselect&id=<#=id#>" class="name" title="<#=typeText#>">[<#=typeText#>]</span>'
            +'  <span class="amount"><#=num#>注x<#=multiple#>倍x<#=moneyUnitText#>=<#=amountText#>元</span>'
            +'  <span data-param="action=del&id=<#=id#>" class="delete unicode-icon-delete"></span>'
            +' </li>',
      //显示内容截取字符串长度
  ...
  var pros = {
    ...
    add: function(order) {
      ...
      if (order['lotterys'] && order['lotterys'].length > 0) {
        ...
        //order['prizeGroup'] = Games.getCurrentGame().getGameConfig().getInstance().getUserPrizeGroup();
        // console.log(Games.getCurrentGame().getGameConfig().getInstance().getUserPrizeGroup())
    ...
    fastOrder: function(order) {
      var me = this;
      order = me.getFastOrder(order);
      Games.getCurrentGameSubmit().submitData( order );
      // reset
      // Games.getCurrentGame().getCurrentGameMethod().reSet();
      // Games.getCurrentGameStatistics().reSet();
      // me.updateData();
    },
    getFastOrder: function(order) {
      var me = this;
      order.id = orderID++;
      order.postParameter = Games.getCurrentGame().getCurrentGameMethod().makePostParameter(order.original, order);
      order = me.rebuildData(order);
      return Games.getCurrentGameSubmit().getFastSubmitData(order);
    },
    //替换某个Order注单对象
    ...
    rebuildData: function(order) {
      ...
      order['moneyUnitText'] = order['moneyUnitText'] || Games.getCurrentGameStatistics().getMoneyUnitData()['text'];
    ...
    checkData: function(order) {
      ...
      //返回对象在数组的索引值index
      //未找到返回-1
      return me.searchSameResult( name, saveArray.join(), moneyUnit, prizeGroup, digitChoose);
    },
    eventProxy: function() {
      var me = this,
    ...
    exeEvent_reselect: function(param) {
      ...
      $('.cart-panel').find('.action-back').trigger(betgame.touchEvent);
    },
    //界面状态更新
    ...
    selectOrderById: function(id) {
      ...
      //设置倍数、元角模式
      Games.getCurrentGameStatistics().getMultipleDom().val(order['multiple']);
      // Games.getCurrentGameStatistics().getMoneyUnitDom().setValue(order['moneyUnit']);
      Games.getCurrentGameStatistics().setMoneyUnitDom(order['moneyUnit']);
     ...
     */
    searchSameResult: function(name, lotteryText, moneyUnit, prizeGroup, digitChoose) {
      var me = this,
        current, dataNum,
        i = 0,
        saveArray = [],
      ...
      for (; i < data.length; i++) {
        ...
        if (
          current.type == name && lotteryText == saveArray.join() &&
          current.moneyUnit == moneyUnit && current.prizeGroup == prizeGroup &&
          current.digitChoose == digitChoose
        ) {
          return i;
        }
      }
...
})(betgame, "GameOrder", betgame.Event);
