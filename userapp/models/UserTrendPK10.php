...
class UserTrendPK10 extends BaseModel {
    ...
    protected static $aCellNums = ['3' => 15];
    // 不同统计类型号码截取的起始下标
    ...
    protected $index = 0;
    protected $ballNum = 0;
    protected $colCount = 15; // 标准列数(球个数 + 分布 + [二星的跨度]) * 10 + [1 (二星的对子)]
    protected $seriesId = 6; // 彩系, 1: SSC, 2: 11-5
...
//        return $result;
    ...
    public function generateTrendData(& $aWnNumbers, & $statistics, & $hotAndCold, & $aOmissionBarStatus, $iNumType) {
        ...
        $aData = [];
        $aTotalYiLou = array_fill(0, $iColumnNum, 0);
        foreach ($aWnNumbers as $key1 => $aIssue) {
            $aYiLou = [];
            // 如果是时时彩, 则按号码位数分割, 11选5则按空格分割
            $oPK10Number = new PK10Number($aIssue['wn_number']);
            $aWinNumber = $oPK10Number->getDigitals();
            $aYiLou[0] = $aIssue['issue'];
            foreach ($aWinNumber as $number) {
                $aYiLou[] = $number;
            }
            $aYiLou[11] = $oPK10Number->getDX(0);
            $aYiLou[12] = $oPK10Number->getDX(1);
            $aYiLou[13] = $oPK10Number->getDX(2);
            $aYiLou[14] = $oPK10Number->getDS(0);
            $aYiLou[15] = $oPK10Number->getDS(1);
            $aYiLou[16] = $oPK10Number->getDS(2);
            $aYiLou[17] = $oPK10Number->getDTS(0, 9);
            $aYiLou[18] = $oPK10Number->getDTS(1, 8);
            $aYiLou[19] = $oPK10Number->getDTS(2, 7);
            $aData[] = $aYiLou;
        }
        return $aData;
    }
