  ...
  <div id="section">
    ...
    <div class="ds-tabs">
      <ul class="nav nav-tabs nav-tabs2">
        <li class="active"><a href="{{route('mobile-station-letters.index')}}">站内信</a></li>
        <li><a href="{{ route('mobile-announcements.index') }}">公告</a></li>
      </ul>
        ...
        <div class="tab-pane fade in active">
          <table class="table table-striped">
            <tbody>
              ...
              <tr>
                <td data-page-tab=".article-page" data-href="{{ route('mobile-station-letters.view', $data->id) }}">
                  <a>{{ $data->msg_title }}</a><br>
                  <small>{{$data->created_at}}</small>&nbsp;&nbsp;&nbsp;&nbsp;
                  <?php if($data->is_readed){ ?>
                  <small class="c-gray">已读</small>
                  <?php }else{ ?>
                  <small data-status class="c-highlight">未读</small>
                  <?php } ?>
                </td>
              </tr>
              @endforeach
    ...
    if( url ){
      $.ajax({
        ...
        success: function(resp){
          ...
          $handler.siblings('[data-status]').text('已读')
            .removeClass('c-highlight').addClass('c-gray');
        },
        complete: function(){
