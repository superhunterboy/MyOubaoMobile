...
class MobileAccountController extends UserAccountController {

    protected $resourceView = 'transfer';
...
//        }
        ...
        $oToUser = UserUser::getObjectByParams(['username'=>$this->params['username']]);
        if(!is_object($oToUser)){
            return $this->goBack('error', __('_user.missing-user'));
        }
        $iToUserId = $oToUser->id;
        ...
        if ($this->params['desc'] == self::DESC_DEPOSIT_BY_PARENT || $bCheckRole) {
            ...
            return $this->goBack('error', '请选择转账说明');
        }
