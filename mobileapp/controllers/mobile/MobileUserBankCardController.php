...
class MobileUserBankCardController extends UserUserBankCardController {

    protected $resourceView = 'userbankcard';
...
// 绑卡前先设置资金密码
        if ($oUser->fund_password == null) {
            ...
            Session::put('curPage-UserUser', route('mobile-users.safe-reset-fund-password'));
            return Redirect::route('mobile-users.safe-reset-fund-password', 1);
        }
// 绑卡时，银行卡不能被锁定
        $bLocked = UserUserBankCard::getUserCardsLockStatus($iUserId);
        if ($bLocked) {
        ...
        if ($oUserBankCard->status != UserUserBankCard::STATUS_IN_USE) {
            return Redirect::route('mobile-user-bank-cards.index')->with('error', __('_userbankcard.missing-data'));
        }
        switch ($iStep) {
            case 0:
    ...
    private function validate($iCardId = null) {
        ...
        if (Request::method() == 'POST') {
            ...
            if (!$bValidated) {
                ...
                return Redirect::route('mobile-user-bank-cards.' . ($iCardId ? 'modify-card' : 'bind-card'), $iCardId ? [self::BIND_CARD_STEP1, $iCardId] : self::BIND_CARD_STEP1);
            }
        } else {
...
//如果不是第一次绑卡，则需要验证卡信息
        if (!$iCardId && !$this->isFirstCard && !Session::get(self::CARD_BINDING_STATUS_NAME . self::BIND_CARD_STEP0)) {
            return Redirect::route('mobile-user-bank-cards.bind-card', self::BIND_CARD_STEP0);
        }
// 如果是修改卡信息，则需要验证旧卡信息
        if ($iCardId && !Session::get(self::CARD_BINDING_STATUS_NAME . self::BIND_CARD_STEP0)) {
            return Redirect::route('mobile-user-bank-cards.modify-card', [self::BIND_CARD_STEP0, $iCardId]);
        }
...
//            'account' => 'required|regex:/^[0-9]*$/|between:16,19|confirmed',
        ...
        if (!$validator->passes()) {
            ...
            $sRedirectName = $iCardId ? 'mobile-user-bank-cards.modify-card' : 'mobile-user-bank-cards.bind-card';
...
//判断该银行卡是否已经绑定
        ...
        if (is_object($oUserBankCard)) {
            return Redirect::route('mobile-user-bank-cards.bind-card', self::BIND_CARD_STEP1)->with('error', __('_userbankcard.user-bank-card-exist'));
        }
...
// 绑定银行卡到指定用户
        ...
        if ($bSucc) {
            DB::connection()->commit();
            return Redirect::route('mobile-user-bank-cards.bind-card', self::BIND_CARD_STEP4);
        } else {
            DB::connection()->rollback();
            $this->langVars['reason'] = & $this->model->getValidationErrorString();
            return Redirect::route('mobile-user-bank-cards.bind-card', self::BIND_CARD_STEP1)
                            ->with('error', __('_userbankcard.bind-card-fail', $this->langVars));
        }
    }
...
// 绑定银行卡到指定用户
        ...
        if ($this->model->account != $this->params['account']) {
            ...
            if (is_object($oUserBankCard)) {
                return Redirect::route('mobile-user-bank-cards.modify-card', [self::BIND_CARD_STEP1, $iCardId])->with('error', __('_userbankcard.user-bank-card-exist'));
            }
// 删除原来的绑定卡
...
//创建新的绑定卡
        ...
        if ($bSucc) {
            DB::connection()->commit();
            return Redirect::route('mobile-user-bank-cards.modify-card', [self::BIND_CARD_STEP4, $iCardId]);
        } else {
            DB::connection()->rollback();
            $this->langVars['reason'] = & $this->model->getValidationErrorString();
            return Redirect::route('mobile-user-bank-cards.modify-card', [self::BIND_CARD_STEP1, $iCardId])
                            ->with('error', __('_userbankcard.bind-card-fail', $this->langVars));
        }
    }
