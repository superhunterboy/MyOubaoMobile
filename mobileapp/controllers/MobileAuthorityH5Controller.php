...
class MobileAuthorityH5Controller extends Controller {
    ...
    public function postSignin() {
        ...
        if (count($aRandom) != 2 || (count($aRandom) == 2 && ($aRandom[1] != Session::get($aRandom[0])))) {
            return Redirect::back()->with('error', __('_basic.login-fail-wrong'));
        }
        ...
        if (isset($iLoginTimes) && ($iLoginTimes > 3)) {
            ...
            if ($this->validateCaptchaError($sErrorMsg)) {
                return Redirect::back()->with('error', $sErrorMsg);
            }
        }
        ...
        if ($user->blocked == User::BLOCK_LOGIN) {
            return Redirect::back()->with('error', __('_basic.login-fail-blocked'));
    ...
    public function postSignup() {
        ...
        if ($oPrizeGroup->is_agent && !$oPrizeGroup->is_admin && $sLinkPrizeGroup >= 1950) {
    ...
    public function findPassword() {
        if (Request::method() == 'PUT') {
            ...
            if ($bSucc) {
                return Redirect::route('mobile-auth.login')->with('success', __('_user.password-updated'));
            } else {
