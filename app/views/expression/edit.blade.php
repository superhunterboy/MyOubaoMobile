...
<div class="col-md-12">
    ...
    <div class="panel panel-default">
        <div class=" panel-body">
            ...
            <form method="post" action="{{ route($resource .'.create', $id) }}" accept-charset="UTF-8" class="form-horizontal">
                <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                <div>
                    <div class="form-group">
                        <label for=" " class="col-sm-2 control-label"> 缩写名称：</label>
                        <div class="col-sm-4"><input class="form-control" type="text" name="identifier" value="{{$oExpression->identifier}}"></div>
                        <div class="col-sm-4"></div>
                    </div>
                    <div class="form-group">
                        <label for=" " class="col-sm-2 control-label"> 描述：</label>
                        <div class="col-sm-4"><input class="form-control" type="text" name="description" value="{{$oExpression->description}}"></div>
                        <div class="col-sm-4"></div>
                    </div>
                    <div class="form-group">
                        <label for=" " class="col-sm-2 control-label"> 返还金额：</label>
                        <div class="col-sm-10 j-fee-box">
                            @foreach($aExpressions as $k => $fe)
                            <?php
                            ...
                            <div class="j-add-fee clearfix" style="margin-bottom:2px;">
                                ...
                                <div style=" width:80px; float:left;">
                                    <input class="form-control" type="text" onkeyup="this.value = this.value.replace(/[^0-9].[0-9][0-9]$/g, '')"  value="{{$amount_left}}" name="expression_set[{{$k}}][amount_left]">
                                </div>
                                ...
                                <div style=" width:80px; float:left;">
                                    <input class="form-control" type="text" onkeyup="this.value = this.value.replace(/[^0-9].[0-9][0-9]$/g, '')" value="{{$amount_right}}" name="expression_set[{{$k}}][amount_right]">
                                </div>
                                <!-- 条件区 END -->
                                <label for=" " class="control-label " style="text-align: center; width:120px; float:left;">，所返手续费为：</label>
                                <!-- 取值区 START -->
                                <div  style=" width:60px; float:left;">
                                    <select class="form-control j-select" name="expression_set[{{$k}}][operator]" >
                                ...
                                <div  style=" width:120px; float:left;" class="j-teggle">
                                    @if(isset($fe['y']['=']))
                                    <span class="j-none">
                                        <input style=" width:50px;float:left;" class="form-control" type="text" value="{{$fe['y']['=']}}" name="expression_set[{{$k}}][value]">
                                        <label for=" " class="control-label ">元</label>
                                    </span>
                                    <span class="j-none"style="display: none;">
                                        <select style=" width:50px;float:left;" class="form-control" name="expression_set[{{$k}}][value]" id=" " disabled="">
                                        </select>
                                        <label for=" " class="control-label " ><font color="red"> %</font></label>
                                    </span>
                                    @elseif(isset($fe['y']['%']))
                                    <span class="j-none" style="display: none;">
                                        <input style=" width:50px;float:left;" class="form-control" type="text" value="" name="expression_set[{{$k}}][value]" disabled="">
                                        <label for=" " class="control-label " >元</label>
                                    </span>
                                    <span class="j-none">
                                        <select style=" width:50px;float:left;" class="form-control" name="expression_set[{{$k}}][value]" id=" " >
                                        </select>
                                        <label for=" " class="control-label "><font color="red"> %</font></label>
                                    </span>
                                    @endif
