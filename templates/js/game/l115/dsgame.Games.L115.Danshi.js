		...
		iterator: function(data, filtr) {
			...
			for (var i = 0; i < data.length; i++) {
				if (filtr.test(data.charAt(i))) {
					current = $.trim(data.substr(breakNum, i - breakNum));
					if (current != '') {
						count.push(current.split(' '));
					}
					breakNum = i + 1;
				}
			};
