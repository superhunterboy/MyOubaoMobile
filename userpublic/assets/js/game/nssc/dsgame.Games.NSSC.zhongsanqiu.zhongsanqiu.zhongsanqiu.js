...
(function(host, GameMethod, undefined){
	var defConfig = {
		name:'zhongsanqiu.zhongsanqiu.zhongsanqiu',
		//玩法提示
		tips:'',
		//选号实例
		exampleTip: ''
			...
			$.each(data, function(i){
				...
				html.push('<tr class="'+ trcls +'">');
					...
					$.each(number, function(j){
						...
						number[j] = parseInt(this);
					});
					html.push('</span></td>');
					if( number.length == 5 ){
						var xt = '杂六',
							newNumber = number.slice(1,4);
						newNumber = newNumber.sort(function(a,b){
							return a - b;
						});
						if( newNumber[0] == newNumber[1] && newNumber[0] == newNumber[2] ){
							xt = '豹子';
						}else if( 
							(newNumber[0] == 0 && newNumber[1] == 1 && newNumber[2] == 9) ||
							( (newNumber[0] - newNumber[1]) == -1 &&  (newNumber[1] - newNumber[2]) == -1 )
						){
							xt = '顺子';
						}else if( newNumber[0] == newNumber[1] || newNumber[1] == newNumber[2] ){
							xt = '对子';
						}else if(
							(newNumber[0] == 0 && newNumber[2] == 9) ||
							(newNumber[0] - newNumber[1]) == -1 ||
							(newNumber[1] - newNumber[2]) == -1
						){
							xt = '半顺';
						}
						html.push('<td>'+ xt +'</td>');
	...
	var html_row = [];
		...
		html_row.push('<ul class="ball-content">');
			$.each(['顺子','对子','半顺','杂六'], function(i){				
				html_row.push('<li><a href="javascript:void(0);" data-param="action=ball&value='+ i +'&row=<#=row#>" class="ball-number ball-number-long">'+this+'</a></li>');
