...
(function(host, GameMethod, undefined){
	var defConfig = {
		name:'erxing.zhixuan.qianerkuadu',
		//玩法提示
		tips:'前二直选跨度玩法提示',
		//选号实例
		exampleTip: '前二直选跨度范例'
		...
		miniTrend_createRowHtml:function(){
			...
			$.each(data, function(i){
				...
				html.push('<tr class="'+ trcls +'">');
					...
					html.push('</span></td>');
					xtText = Math.max(parseInt(number[0]), parseInt(number[1])) - Math.min(parseInt(number[0]), parseInt(number[1]));
