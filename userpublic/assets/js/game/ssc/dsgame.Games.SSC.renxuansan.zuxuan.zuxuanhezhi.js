...
(function(host, GameMethod, undefined){
	var defConfig = {
		//名称
		name:'renxuansan.zuxuan.zuxuanhezhi',
		//玩法提示
		tips:'直选复式玩法提示',
		//选号实例
		exampleTip: '五星直选复式范例',
	...
	var pros = {
		...
		heizhi_combs_calc: function(idx){
			var hash = [0,1,2,2,4,5,6,8,10,11,13,14,14,15,15,14,14,13,11,10,8,6,5,4,2,2,1];
			var ret = new Array(hash[idx]);
			return ret;
		}
	...
	var html_row = [];
		...
		html_row.push('<ul class="ball-content">');
			for(var k=0; k<27; k++){
				if( k==0 ){
					html_row.push('<li style="display:none;"><a class="ball-number" data-param="action=ball&value='+ k +'&row=<#=row#>" href="javascript:void(0);">'+ k +'</a></li>');
				}else{
					html_row.push('<li><a class="ball-number" data-param="action=ball&value='+ k +'&row=<#=row#>" href="javascript:void(0);">'+ k +'</a></li>');
				}
			}
		html_row.push('</ul>');
