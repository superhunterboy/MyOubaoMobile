...
(function(host, GameMethod, undefined){
	var defConfig = {
		//名称
		name:'renxuansan.zuxuan.zusanfushi',
		//玩法提示
		tips:'直选复式玩法提示',
		//选号实例
		exampleTip: '五星直选复式范例',
	...
	var pros = {
		...
		getLottery:function(isGetNum){
			...
			if( result.length >= ballNumNeed ){
				$.each(result, function(i,n){
					var arr1 = [n,n],
						arr2 = result.slice(0,i).concat( result.slice(i+1) );
					finalResult = finalResult.concat( me._combinations(arr1, arr2) );
				});
		...
		_combinations: function(arr1, arr2){
			var ret = [];
			$.each(arr2, function(i, n){
				ret.push( arr1.concat([n]) );
			});
			return ret;
		}
